 ...
 *	  pmc_offset(scratch6), 15:8)
 *     is to:
 *         copy bits 7:0 of sdram_offset.emc_clock_source to
 *         bits 15:8 of scratch6 register.
 *
 *   The first parameter of pack determines the type of src:
 *     TYPE_SDRAM: src is from SDRAM parameter,
 *     TYPE_CLKRST:  src is from CLKRST register,
...
#define pmc_offset(member) offsetof(struct tegra_pmc_regs, member)
#define clkrst_offset(member) offsetof(struct clk_rst_ctlr, member)
...
#define m(offset, src, scratch, dst)	\
	pack(TYPE_CLKRST, clkrst_offset(offset), src, pmc_offset(scratch), dst)

#define c(const, src, scratch, dst)	\
	pack(TYPE_CONST, const, src, pmc_offset(scratch), dst)

static struct pack_fields first[] = {
...
static struct pack_fields ddr3[] = {
	...
	c(0, 31:0, scratch116, 31:0),
	c(0, 31:0, scratch117, 31:0),
};

static struct pack_fields lpddr2[] = {
...
static struct pack_fields last[] = {
	...
	c(3, 1:0, pllp_wb0_override, 12:11),
};

/* Executes PMC.dst_offset.dst_bits = src.src_offset.src_bits; for one pack. */
static void do_pack(struct pack_fields *pack, const struct sdram_params *sdram)
{
	u32 val, reg;

	switch (pack->src_type) {
	case TYPE_SDRAM:
		val = readl((void *)sdram + pack->src_offset);
		break;
	case TYPE_CLKRST:
		val = readl((void *)TEGRA_CLK_RST_BASE + pack->src_offset);
		break;
	case TYPE_CONST:
		val = pack->src_offset;
		break;
	default:
		die("Pack list contains invalid src_type!\n");
		return;
	}
	...
	reg = readl((void *)TEGRA_PMC_BASE + pack->dst_offset);
	reg &= ~(pack->dst_mask << pack->dst_shift);
	reg |= val;
	writel(reg, (void *)TEGRA_PMC_BASE + pack->dst_offset);
}
...
void sdram_lp0_save_params(const struct sdram_params *sdram)
{
	...
	for (pack = first; pack < first + ARRAY_SIZE(first); pack++)
		do_pack(pack, sdram);

	switch (sdram->MemoryType) {
	case NvBootMemoryType_LpDdr2:
		for (pack = lpddr2; pack < lpddr2 + ARRAY_SIZE(lpddr2); pack++)
			do_pack(pack, sdram);
		break;
	case NvBootMemoryType_Ddr3:
		for (pack = ddr3; pack < ddr3 + ARRAY_SIZE(ddr3); pack++)
			do_pack(pack, sdram);
		break;
	default:
	...
	for (pack = last; pack < last + ARRAY_SIZE(last); pack++)
		do_pack(pack, sdram);
}
