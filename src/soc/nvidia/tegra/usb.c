...
static void usb_ehci_reset_and_prepare(struct usb_ctlr *usb, enum usb_phy_type type)
{
	...
	writel(3 << 0, &usb->usb_mode);	/* Controller mode: HOST */
	writel(type << 29, &usb->lpm_ctrl);	/* Parallel transceiver selct */
	writel(0x10 << 16, &usb->tx_fill_tuning);  /* Tx FIFO Burst thresh */
}
