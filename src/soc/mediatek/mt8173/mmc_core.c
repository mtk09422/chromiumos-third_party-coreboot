...
static void mmc_prof_card_init(void *data, ulong id, ulong counts)
{
    int err = (int)data;
    if (!err) {
        printk(BIOS_DEBUG, "[SD%d] Init Card, %d counts, %d us\n",
...
static void mmc_prof_read(void *data, ulong id, ulong counts)
{
    ...
    printk(BIOS_DEBUG, "[SD%d] Read %d bytes, %d counts, %d us, %d KB/s, Avg: %d KB/s\n",
...
static void mmc_prof_write(void *data, ulong id, ulong counts)
{
    ...
    printk(BIOS_DEBUG, "[SD%d] Write %d bytes, %d counts, %d us, %d KB/s, Avg: %d KB/s\n",
        perf->host->id, blkcnt * blksz, counts,
        counts * 30 + counts * 16960 / 32768,
        blkcnt * blksz * 32 / (counts ? counts : 1),
        ((rpt->total_size / 1024) * 32768) / rpt->total_time);
...
static int mmc_decode_csd(struct mmc_card *card)
{
	...
	if (mmc_card_sd(card)) {
        ...
        csd->write_prot_grpsz = MSDC_DECODE_BITS(resp, 32, 5);
...
static void mmc_stuff_buff(u8 * buf)
{
	memset(buf, 0, 512);
	buf[0] = 0x10;
	buf[1] = 0x06;
...
int mmc_get_sandisk_fwid(int id, u8 * buf)
{
	...
	while (state != 4) {
		...
		DEBUG_MMC(BIOS_DEBUG, "check card state<%d>\n", state);
		if (state == 5 || state == 6) {
			DEBUG_MMC(BIOS_DEBUG, "state<%d> need cmd12 to stop\n",
				state);
			...
			DEBUG_MMC(BIOS_DEBUG, "state<%d> card is busy\n",
				state);
			udelay(100 * 1000);
		} else if (state != 4) {
			DEBUG_MMC(BIOS_DEBUG, "state<%d> ??? \n", state);
			return MMC_ERR_INVALID;
		}
	}
	mmc_stuff_buff(buf);
	...
	if (err) {
		DEBUG_MMC(BIOS_DEBUG,
			"[SD%d] Fail to send(CMD50) sandisk fwid %d\n",
			host->id, err);
		return err;
	}
	...
	if (err) {
		DEBUG_MMC(BIOS_DEBUG,
			"[SD%d] Fail to get(CMD21) sandisk fwid %d\n",
			host->id, err);
		return err;
	}
#else
...
int mmc_init(int id)
{
	...
	printk(BIOS_DEBUG, "[%s]: msdc%d start in Preloader\n", __func__, id);
	BUG_ON(id >= NR_MMC);

	host = &sd_host[id];
	card = &sd_card[id];
    printk(BIOS_DEBUG, "[%s]: msdc%d, start mmc_init_host() in Preloader\n", __func__, id);
	err = mmc_init_host(host, id);
    if (err == MMC_ERR_NONE){
        printk(BIOS_DEBUG, "[%s]: msdc%d start mmc_init_card() in Preloader\n", __func__, id);
		err = mmc_init_card(host, card);
    }

    printk(BIOS_DEBUG, "[%s]: msdc%d init done in Preloader, err=%d\n", __func__, id, err);
    return err;
}
