...
static void mt_gpio_set_default_chip(void)
{
	...
	mt_set_gpio_pull_select(GPIO119, GPIO119_PULL);
	mt_set_gpio_pull_select(GPIO120, GPIO120_PULL);
...
void mt_gpio_set_default_ext(void)
{
	...
	for (idx = 0; idx < sizeof(pReg->dir) / sizeof(pReg->dir[0]); idx++) {
		...
		gpioext_write(&pReg->dir[idx].val, val);
	}

	for (idx = 0; idx < sizeof(pReg->pullen) / sizeof(pReg->pullen[0]); idx++) {
		val = gpioext_init_pullen_data[idx];
		gpioext_write(&pReg->pullen[idx].val, val);
	}

	for (idx = 0; idx < sizeof(pReg->pullsel) / sizeof(pReg->pullsel[0]); idx++) {
		val = gpioext_init_pullsel_data[idx];
		gpioext_write(&pReg->pullsel[idx].val, val);
	}

	for (idx = 0; idx < sizeof(pReg->dinv) / sizeof(pReg->dinv[0]); idx++) {
		val = gpioext_init_dinv_data[idx];
		gpioext_write(&pReg->dinv[idx].val, val);
	}

	for (idx = 0; idx < sizeof(pReg->dout) / sizeof(pReg->dout[0]); idx++) {
		val = gpioext_init_dout_data[idx];
		gpioext_write(&pReg->dout[idx].val, val);
	}

	for (idx = 0; idx < sizeof(pReg->mode) / sizeof(pReg->mode[0]); idx++) {
		val = gpioext_init_mode_data[idx];
		gpioext_write(&pReg->mode[idx].val, val);
	}
...
static void mt_gpio_set_dsel(u8 md_power)
{
	...
	if (md_power == GPIO_VIO18) {
		...
		write16(&pReg->exmd_ctrl[0].rst, 0x3F5);
		write16(&pReg->exmd_ctrl[0].set, 0xA);
	} else if (md_power == GPIO_VIO28) {
		/* exmd_ctrl[9:4] = b`001100, [3:0] = b`1010 */
		write16(&pReg->exmd_ctrl[0].rst, 0x335);
		write16(&pReg->exmd_ctrl[0].set, 0xCA);
	}
	GPIOVER("EXMD 0x%x\n", read32(&pReg->exmd_ctrl[0].val));

	/* other R/TDSEL */
	write16(&pReg->msdc2_ctrl5.set, 0xA);    // msdc1_ctrl4 , bit[3:0] = b`1010
	write16(&pReg->msdc2_ctrl5.rst, 0x5);
}
