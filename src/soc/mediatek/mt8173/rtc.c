...
static u16 get_frequency_meter(u16 val, u16 measureSrc, u16 window_size)
{
	...
	RTC_Write(PMIC_RG_TOP_RST_CON, 0x0100);	/* FQMTR reset */
	while (!(RTC_Read(PMIC_RG_FQMTR_CON2) == 0)
	       && (0x8 & RTC_Read(PMIC_RG_FQMTR_CON0)) == 0x8) ;
	RTC_Write(PMIC_RG_TOP_RST_CON, 0x0000);	/* FQMTR normal */
	...
	RTC_Write(PMIC_RG_FQMTR_CON0, 0x8000 | measureSrc);

	udelay(1000);
	stopwatch_init_usecs_expire(&sw, 1000 * USECS_PER_MSEC);
	while ((0x8 & RTC_Read(PMIC_RG_FQMTR_CON0)) == 0x8) {
		if (stopwatch_expired(&sw)) {
...
static u16 eosc_cali(void)
{
	...
	printk(BIOS_INFO, "[RTC] EOSC_Cali: TOP_CKCON1=0x%x\n",
	       RTC_Read(PMIC_RG_TOP_CKCON1));

	int left = 0x24C0, right = 0x24DF;

	RTC_Write(PMIC_RG_TOP_CKCON2, (RTC_Read(PMIC_RG_TOP_CKCON2) | 0x8000));
	while (left <= (right)) {
		middle = (right + left) / 2;
		if (middle == left)
			break;
...
static void rtc_osc_init(void)
{
	...
	if (rtc_get_xosc_mode()) {
		...
		u16 val;
		RTC_Write(PMIC_RG_TOP_CKTST2, 0x80);
		val = eosc_cali();
...
static void rtc_measure_four_clock(u16 * result)
{
	...
	RTC_Write(PMIC_RG_TOP_CKTST2, (RTC_Read(PMIC_RG_TOP_CKTST2) & ~0x00C0));
	RTC_Write(PMIC_RG_TOP_CKCON2, (RTC_Read(PMIC_RG_TOP_CKCON2) | 0x8000));
	window_size = 4;
	udelay(1000);
	...
	RTC_Write(PMIC_RG_TOP_CKTST2, ((RTC_Read(PMIC_RG_TOP_CKTST2) | 0x0080) & ~0x0040));
	RTC_Write(PMIC_RG_TOP_CKCON2, (RTC_Read(PMIC_RG_TOP_CKCON2) | 0x8000));
	window_size = 4;
	udelay(1000);
...
static void rtc_switch_mode(bool XOSC, bool recovery)
{
	if (XOSC) {
		if (recovery) {
			...
			RTC_Write(PMIC_RG_CHRSTATUS,
				  ((RTC_Read(PMIC_RG_CHRSTATUS) | 0x0800) & ~0x0200));
		}
		...
		if (recovery) {
			...
			RTC_Write(PMIC_RG_CHRSTATUS, (RTC_Read(PMIC_RG_CHRSTATUS) | 0x0A00));
		}
...
static bool rtc_init_after_recovery(void)
{
	...
	RTC_Write(PMIC_RG_CHRSTATUS, (RTC_Read(PMIC_RG_CHRSTATUS) & ~0x0800));
...
static void dcxo_setting_correct_frequency(void)
{
	...
	tmp = RTC_Read(PMIC_RG_DCXO_CON2);
	RTC_Write(PMIC_RG_DCXO_CON2,
		  tmp & (~(PMIC_RG_DCXO_C2_UNTRIM_MASK <<
			   PMIC_RG_DCXO_C2_UNTRIM_SHIFT)));

	tmp = RTC_Read(PMIC_RG_DCXO_MANUAL_CON1);
	RTC_Write(PMIC_RG_DCXO_MANUAL_CON1,
...
void rtc_bbpu_power_on(void)
{
	...
	RTC_Write(PMIC_RG_TOP_CKPDN2,RTC_Read(PMIC_RG_TOP_CKPDN2)|0x4000);
	if (1 == RTC_2SEC_REBOOT_ENABLE)
