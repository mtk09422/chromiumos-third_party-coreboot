...
inline u8 RegisterSetField(DRAMC_CTX_T *p, u32 addr, u32 value, u32 mask, u32 pos)
{
	u8  ucstatus = 0;
	u32 u4value;
...
static inline u8 sel_ph_and_write_latency_setting(DRAMC_CTX_T *p)
{
	...
	ucstatus |= RegisterWriteVal(p, 0x1e0, 0x220fffff);
	ucstatus |= RegisterWriteVal(p, 0x23c, 0x220fffff);

	ucstatus |= RegisterWriteVal(p, 0x430, 0x10f010f0);
	ucstatus |= RegisterWriteVal(p, 0x434, 0xffffffff);
	ucstatus |= RegisterWriteVal(p, 0x438, 0xffffffff);
	ucstatus |= RegisterWriteVal(p, 0x43c, 0x0000001f);
...
static inline u8 MEMPLL_EN(DRAMC_CTX_T *p, u32 TYPE, u32 bit)
{
	...
	if (TYPE == ENABLE) {
		/* MEMPLL2_EN */
		ucstatus |= RegisterSetBit(p, 0x614, bit);
		/* MEMPLL4_EN */
		ucstatus |= RegisterSetBit(p, 0x62c, bit);
		/* MEMPLL05_2_EN */
		ucstatus |= RegisterSetBit(p, 0x664, bit);
		/* MEMPLL05_3_EN */
		ucstatus |= RegisterSetBit(p, 0x670, bit);
		/* MEMPLL3_EN */
		ucstatus |= RegisterSetBit(p, 0x620, bit);
		/* MEMPLL05_4_EN */
		ucstatus |= RegisterSetBit(p, 0x67c, bit);
	} else { /* Disable Reg. */
		/* MEMPLL2_EN */
		ucstatus |= RegisterClrBit(p, 0x614, bit);
		/* MEMPLL4_EN */
		ucstatus |= RegisterClrBit(p, 0x62c, bit);
		/* MEMPLL05_2_EN */
		ucstatus |= RegisterClrBit(p, 0x664, bit);
		/* MEMPLL05_3_EN */
		ucstatus |= RegisterClrBit(p, 0x670, bit);
		/* MEMPLL3_EN */
		ucstatus |= RegisterClrBit(p, 0x620, bit);
		/* MEMPLL05_4_EN */
		ucstatus |= RegisterClrBit(p, 0x67c, bit);
	}
...
static inline u8 MEMPLL_FB_MCK_SEL(DRAMC_CTX_T *p, u32 TYPE, u32 bit)
{
	...
	if (TYPE == ENABLE) {
		/* MEMPLL2_FB_MCK_SEL */
		ucstatus |= RegisterSetBit(p, 0x618, bit);
		/* MEMPLL4_FB_MCK_SEL */
		ucstatus |= RegisterSetBit(p, 0x630, bit);
		/* MEMPLL05_2_FB_MCK_SEL */
		ucstatus |= RegisterSetBit(p, 0x668, bit);
		/* MEMPLL05_3_FB_MCK_SEL */
		ucstatus |= RegisterSetBit(p, 0x674, bit);
		/* MEMPLL3_FB_MCK_SEL */
		ucstatus |= RegisterSetBit(p, 0x624, bit);
		/* MEMPLL05_4_FB_MCK_SEL */
		ucstatus |= RegisterSetBit(p, 0x680, bit);
	}
...
DRAM_STATUS_T MemPllPreInit(DRAMC_CTX_T *p)
{
	...
	/* SPM_MODIFY */
	p->channel = CHANNEL_A;
	ucstatus |= RegisterWriteVal(p, 0x5c8, 0x1111ff11);
	ucstatus |= RegisterWriteVal(p, 0x5cc, 0x11511111);
	...
	if ((p->package == PACKAGE_POP) || (p->package == PACKAGE_SBS)) {
		...
		/* SPM_MODIFY */
		/* PLL: LEW_SIMULATION */
		ucstatus |= RegisterWriteVal(p, 0x640, 0x09008023);

		/* PLL: LEW_SIMULATION */
		/* SPM control clock enable */
		ucstatus |= RegisterWriteVal(p, 0x690, 0x09008004);
		...
		/* ALLCLK_EN */
		p->channel = CHANNEL_A;
		ucstatus |= RegisterSetBit(p, 0x640, 4);
		...
		p->channel = CHANNEL_A;
		ucstatus |= RegisterClrBit(p, 0x63c, 2);
		ucstatus |= RegisterClrBit(p, 0x63c, 1);
		...
		/* SPM control clock enable */
		p->channel = CHANNEL_A;
		ucstatus |= RegisterClrBit(p, 0x640, 15);
		ucstatus |= RegisterClrBit(p, 0x640, 0);
		...
		/* [0]FDIV2=1 */
		p->channel = CHANNEL_A;
		ucstatus |= RegisterSetBit(p, 0x07c, 0);
		...
		p->channel = CHANNEL_A;
		ucstatus |= RegisterWriteVal(p, 0x5c0, 0x21271b03);
		ucstatus |= RegisterWriteVal(p, 0x5c4, 0x5096061e);
		...
		p->channel = CHANNEL_A;
		ucstatus |= RegisterSetBit(p, 0x640, 7);
		ucstatus |= RegisterSetBit(p, 0x640, 24);
		ucstatus |= RegisterClrBit(p, 0x640, 3);
...
DRAM_STATUS_T MemPllInit(DRAMC_CTX_T *p)
{
	...
	if ((p->package == PACKAGE_POP) || (p->package == PACKAGE_SBS)) {
		...
		p->channel = CHANNEL_A;
		ucstatus |= RegisterWriteVal(p, 0x638, 0x00000000);
		ucstatus |= RegisterWriteVal(p, 0x688, 0x00000000);
		...
		p->channel = CHANNEL_A;
		ucstatus |= RegisterWriteVal(p, 0x60c, 0xd0000000);
		ucstatus |= RegisterWriteVal(p, 0x65c, 0xd0000000);
		...
		p->channel = CHANNEL_A;
		ucstatus |= RegisterWriteVal(p, 0x614, 0x07800000 | pattern1);
		ucstatus |= RegisterWriteVal(p, 0x618, 0x4c00c000 | pattern2);
		ucstatus |= RegisterWriteVal(p, 0x61c, 0x00020001 | pattern3);

		/* mempll 4 config */
		ucstatus |= RegisterWriteVal(p, 0x62c, 0x07800000 | pattern1);
		ucstatus |= RegisterWriteVal(p, 0x630, 0x4c00c000 | pattern2);
		ucstatus |= RegisterWriteVal(p, 0x634, 0x00000001 | pattern3);

		/* mempll05 2  config */
		ucstatus |= RegisterWriteVal(p, 0x664, 0x07800000 | pattern1);
		ucstatus |= RegisterWriteVal(p, 0x668, 0x4c00c000 | pattern2);
		ucstatus |= RegisterWriteVal(p, 0x66c, 0x00020001 | pattern3);

		/* mempll05 3 config */
		ucstatus |= RegisterWriteVal(p, 0x670, 0x07800000 | pattern1);
		ucstatus |= RegisterWriteVal(p, 0x674, 0x4c00c000 | pattern2);
		ucstatus |= RegisterWriteVal(p, 0x678, 0x00000001 | pattern3);

		/* mempll 3 */
		/* (Enable signal tie together.*/
		/*  Sim error due to unknown dividor. E2 will fix.) */
		ucstatus |= RegisterWriteVal(p, 0x620, 0x07800000 | pattern1);
		ucstatus |= RegisterWriteVal(p, 0x624, 0x4c00c000 | pattern2);
		ucstatus |= RegisterWriteVal(p, 0x628, 0x00020001 | pattern3);

		/* mempll05 4 */
		/* (Enable signal tie together. */
		/*  Sim error due to unknown dividor. E2 will fix.) */
		ucstatus |= RegisterWriteVal(p, 0x67c, 0x07800000 | pattern1);
		ucstatus |= RegisterWriteVal(p, 0x680, 0x4c00c000 | pattern2);
		ucstatus |= RegisterWriteVal(p, 0x684, 0x00020001 | pattern3);
		...
		p->channel = CHANNEL_A;
		ucstatus |= RegisterClrBit(p, 0x688, 0);
		...
		ucstatus |= RegisterWriteVal(p, 0x60c, 0xd0000040);
		/* MEMPLL_BIAS05_EN = 1 : 0x65c[6] */
		ucstatus |= RegisterWriteVal(p, 0x65c, 0xd0000040);
		...
		ucstatus |= RegisterSetBit(p, 0x60c, 7);
		/* MEMPLL05_BIAS_LPF_EN=1 : 0x65c[7] */
		ucstatus |= RegisterSetBit(p, 0x65c, 7);
		...
		ucstatus |= RegisterSetBit(p, 0x610, 26);
		...
		ucstatus |= RegisterClrBit(p, 0x610, 26);
		...
		*(volatile unsigned int *)(0x10006010) &= ~(0x08000000);
		...
		/* PLL: LEW_SIMULATION */
		p->channel = CHANNEL_A;
		ucstatus |= RegisterWriteVal(p, 0x640, 0x090000B1);
		/* SPM control clock enable */
		ucstatus |= RegisterWriteVal(p, 0x640, 0x090000B2);
		ucstatus |= RegisterWriteVal(p, 0x640, 0x090000B1);
...
DRAM_STATUS_T DramcPreInit(DRAMC_CTX_T *p, EMI_SETTINGS *emi_set)
{
	...
	if (u4WL == 5) {
		/* WL5 */
		ucstatus |= RegisterWriteVal(p, 0x41c, 0x11112222);
		ucstatus |= RegisterWriteVal(p, 0x420, 0x11112222);
		ucstatus |= RegisterWriteVal(p, 0x424, 0x11112222);
		ucstatus |= RegisterWriteVal(p, 0x428, 0xffff5555);
		ucstatus |= RegisterWriteVal(p, 0x42c, 0x00ff0055);
	} else if (u4WL == 6) {
		/* WL6 */
		ucstatus |= RegisterWriteVal(p, 0x41c, 0x22222222);
		ucstatus |= RegisterWriteVal(p, 0x420, 0x22222222);
		ucstatus |= RegisterWriteVal(p, 0x424, 0x22222222);
		ucstatus |= RegisterWriteVal(p, 0x428, 0x5555ffff);
		ucstatus |= RegisterWriteVal(p, 0x42c, 0x005500ff);
	} else {
		/* WL8 */
		ucstatus |= RegisterWriteVal(p, 0x41c, 0x33333333);
		ucstatus |= RegisterWriteVal(p, 0x420, 0x33333333);
		ucstatus |= RegisterWriteVal(p, 0x424, 0x33333333);
		ucstatus |= RegisterWriteVal(p, 0x428, 0x5555ffff);
		ucstatus |= RegisterWriteVal(p, 0x42c, 0x005500ff);
	}

	/* CS_ADV not defined */
	ucstatus |= RegisterWriteVal(p, 0x410, 0x04955555);
	ucstatus |= RegisterWriteVal(p, 0x418, 0x00000432);
	...
	/* 0.5T? */
	ucstatus |= RegisterWriteVal(p, 0x1f8, emi_set->DRAMC_ACTIM05T_VAL);
...
DRAM_STATUS_T DramcInit(DRAMC_CTX_T *p, EMI_SETTINGS *emi_set)
{
	...
	if (p->dram_type == TYPE_LPDDR3) {
		/* SPM_MODIFY defined */
		ucstatus |= RegisterWriteVal(p, 0x5c8, 0x00001010);
		ucstatus |= RegisterWriteVal(p, 0x5cc, 0x00000000);

		/* 1800Mbps */
		ucstatus |= RegisterWriteVal(p, 0x008, 0x00403300);

		/* COMBO_MCP defined */
		val = emi_set->DRAMC_TEST2_4_VAL;
		ucstatus |= RegisterWriteVal(p, 0x048, val);
		ucstatus |= RegisterWriteVal(p, 0x08c, 0x00e00000);

		/* COMBO_MCP & DUAL_RANKS defined */
		if (uiDualRank) {
			val = emi_set->DRAMC_RKCFG_VAL | 0x00000001;
			ucstatus |= RegisterWriteVal(p, 0x110, val);
		} else {
		...
		ucstatus |= RegisterWriteVal(p, 0x0d8, 0x40100110);
		ucstatus |= RegisterWriteVal(p, 0x0e4, 0x00000001);

		if (p->frequency > 800) {
			/* Reg.138[4] tCKEH/tCKEL extend 1T */
			ucstatus |= RegisterWriteVal(p, 0x138, 0x80000c10);
		} else {
			ucstatus |= RegisterWriteVal(p, 0x138, 0x80000c00);
		}

		/* FTTEST_ZQONLY not defined */
		val = 0x99009900 | (DRIVING_DS2_0<<20) | (DRIVING_DS2_0<<4);
		ucstatus |= RegisterWriteVal(p, 0x0b8, val);
		val = 0x99009900 | (DRIVING_DS2_0<<20) | (DRIVING_DS2_0<<4);
		ucstatus |= RegisterWriteVal(p, 0x0bc, val);

		ucstatus |= RegisterWriteVal(p, 0x090, 0x00000000);
		ucstatus |= RegisterWriteVal(p, 0x094, 0x40404040);

		/* [3:0] move to 0x80[4:0]. This value should be DATLAT-3 */
		/* It is used for runtime RX DQ/DQS K (not use??). */
		ucstatus |= RegisterWriteVal(p, 0x0e0, 0x15000000);
		ucstatus |= RegisterWriteVal(p, 0x118, 0x00000005);

		/* [25] DATLAT4 */
		ucstatus |= RegisterWriteVal(p, 0x0f0, 0x02000000);
		ucstatus |= RegisterWriteVal(p, 0x0f4, 0x11000000);
		ucstatus |= RegisterWriteVal(p, 0x168, 0x00000080);
		/* Only enable clock pad 0. */
		ucstatus |= RegisterWriteVal(p, 0x130, 0x10000000);
		udelay(1);

		/* COMBO_MCP defined */
		val = emi_set->DRAMC_CONF1_VAL;
		ucstatus |= RegisterWriteVal(p, 0x004, val);
		ucstatus |= RegisterWriteVal(p, 0x124, 0xc0000011);
		ucstatus |= RegisterWriteVal(p, 0x1c0, 0x00000000);

		/* COMBO_MCP defined */
		val = emi_set->DRAMC_ACTIM_VAL;
		ucstatus |= RegisterWriteVal(p, 0x000, val);
		val = emi_set->DRAMC_MISCTL0_VAL;
		ucstatus |= RegisterWriteVal(p, 0x0fc, val);
		ucstatus |= RegisterWriteVal(p, 0x1ec, 0x00100000);

		/* COMBO_MCP defined */
		val = emi_set->DRAMC_DDR2CTL_VAL;
		ucstatus |= RegisterWriteVal(p, 0x07c, val);

		ucstatus |= RegisterWriteVal(p, 0x080, 0x00000be0);
		ucstatus |= RegisterWriteVal(p, 0x028, 0xf1000000);
		/* COMBO_MCP */
		val = emi_set->DRAMC_ACTIM1_VAL;
		ucstatus |= RegisterWriteVal(p, 0x1e8, val);

		ucstatus |= RegisterWriteVal(p, 0x158, 0x00000000);
		/* Disable ODT before ZQ calibration. */
		ucstatus |= RegisterWriteVal(p, 0x054, 0x00000001);
		ucstatus |= RegisterWriteVal(p, 0x0e4, 0x00000005);
		...
		ucstatus |= RegisterWriteVal(p, 0x088, val);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000001);

		udelay(10);	/* Wait >=10us if not check DAI. */

		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000000);
		...
		ucstatus |= RegisterWriteVal(p, 0x088, 0x00010003);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000001);
		udelay(1);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000000);
		...
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000000);
		/* MR11, ODT disable. */
		ucstatus |= RegisterWriteVal(p, 0x088, 0x0000000b);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000001);
		udelay(1);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000000);
		...
		ucstatus |= RegisterWriteVal(p, 0x0f4, 0x11100000);
		...
		if (uiDualRank) {
			...
			/* MR3, driving stregth */
			/* for testing, set to max */
			ucstatus |= RegisterWriteVal(p, 0x088, 0x10010003);
			ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000001);
			udelay(1);
			ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000000);
			...
			/* MR11, ODT disable. */
			ucstatus |= RegisterWriteVal(p, 0x088, 0x1000000b);
			ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000001);
			udelay(1);
			ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000000);
		...
		ucstatus |= RegisterWriteVal(p, 0x0f4, val);

		/* COMBO_MCP defined */
		val = emi_set->DRAMC_PD_CTRL_VAL;
		ucstatus |= RegisterWriteVal(p, 0x1dc, val);

		ucstatus |= RegisterWriteVal(p, 0x0e4, 0x00000001);
		ucstatus |= RegisterWriteVal(p, 0x1ec, 0x00100001);
		ucstatus |= RegisterWriteVal(p, 0x084, 0x00000a56);
		ucstatus |= RegisterWriteVal(p, 0x00c, 0x00000000);

		/* disable DQSICALI_NEW [23] (HW RX window calibration) */
		/* COMBO_MCP defined */
		val = emi_set->DRAMC_TEST2_3_VAL;
		ucstatus |= RegisterWriteVal(p, 0x044, val);

		/* COMBO_MCP defined */
		val = emi_set->DRAMC_CONF2_VAL;
		ucstatus |= RegisterWriteVal(p, 0x008, val);

		ucstatus |= RegisterWriteVal(p, 0x010, 0x00000000);

		/* DISABLE_DRVREF defined */
		ucstatus |= RegisterWriteVal(p, 0x100, 0x00000000);

		ucstatus |= RegisterWriteVal(p, 0x01c, 0x12121212);
		ucstatus |= RegisterWriteVal(p, 0x0f8, 0x00000000);

		/* CLKTDN_ENABLE defined */
		ucstatus |= RegisterSetBit(p, 0x0d4, 1);	/* CLKTDN */
		ucstatus |= RegisterSetBit(p, 0x0d4, 31);	/* DS3TDN */
		ucstatus |= RegisterSetBit(p, 0x0d4, 29);	/* DS2TDN */
		ucstatus |= RegisterSetBit(p, 0x0d4, 27);	/* DS1TDN */
		ucstatus |= RegisterSetBit(p, 0x0d4, 25);	/* DS0TDN */

		/* DISABLE_PERBANK_REFRESH defined */
		ucstatus |= RegisterClrBit(p, 0x110, 7);
		...
		/* Duty default value. */
		ucstatus |= RegisterWriteVal(p, 0x148, 0x10010000);

		if (uiDualRank == 0) {
			/* Single rank. CKE1 always off. [21]=1 */
			ucstatus |= RegisterSetBit(p, 0xf4, 21);
		}
		...
		if (p->channel == CHANNEL_A) {
			if (uiDualRank) {
				/* DQS RX input delay */
				ucstatus |= RegisterWriteVal(p, 0x018,
								0x08080908);
				/* Need to adjust further. */
				ucstatus |= RegisterWriteVal(p, 0x01c,
								0x08080908);
				/* DQ input delay */
				ucstatus |= RegisterWriteVal(p, 0x210,
								0x01010300);
				ucstatus |= RegisterWriteVal(p, 0x214,
								0x06030002);
				ucstatus |= RegisterWriteVal(p, 0x218,
								0x01010201);
				ucstatus |= RegisterWriteVal(p, 0x21c,
								0x03020002);
				ucstatus |= RegisterWriteVal(p, 0x220,
								0x00010103);
				ucstatus |= RegisterWriteVal(p, 0x224,
								0x02010201);
				ucstatus |= RegisterWriteVal(p, 0x228,
								0x02040200);
				ucstatus |= RegisterWriteVal(p, 0x22c,
								0x02020201);
			} else {
				/* DQS RX input delay */
				ucstatus |= RegisterWriteVal(p, 0x018,
		...
		ucstatus |= RegisterWriteVal(p, 0x430, 0x00f000f0);
		ucstatus |= RegisterWriteVal(p, 0x434, 0xffffffff);
		ucstatus |= RegisterWriteVal(p, 0x438, 0xffffffff);
		ucstatus |= RegisterWriteVal(p, 0x43c, 0x0000001f);
		ucstatus |= RegisterWriteVal(p, 0x400, 0x11111111);
		ucstatus |= RegisterWriteVal(p, 0x404, 0x01312111);
		ucstatus |= RegisterWriteVal(p, 0x408, 0x11111111);
		ucstatus |= RegisterWriteVal(p, 0x40c, 0x11111111);
		ucstatus |= RegisterWriteVal(p, 0x410, 0x05d55555);
		ucstatus |= RegisterWriteVal(p, 0x418, 0x00000217);
		ucstatus |= RegisterWriteVal(p, 0x41c, 0x33333333);
		ucstatus |= RegisterWriteVal(p, 0x420, 0x33333333);
		ucstatus |= RegisterWriteVal(p, 0x424, 0x33333333);
		ucstatus |= RegisterWriteVal(p, 0x428, 0x5555ffff);
		ucstatus |= RegisterWriteVal(p, 0x42c, 0x005500ff);
		ucstatus |= RegisterWriteVal(p, 0x1f8, 0x040006e4);
		ucstatus |= RegisterWriteVal(p, 0x1e0, 0x08000000);
		ucstatus |= RegisterWriteVal(p, 0x23c, 0x00000000);
		...
		/* ========dramc_init============ */
		ucstatus |= RegisterWriteVal(p, 0x048, 0x2300d10d);
		ucstatus |= RegisterWriteVal(p, 0x08c, 0x00e00000);
		ucstatus |= RegisterWriteVal(p, 0x110, 0x00011400);
		/* [11:10] DQIENQKEND 01 -> 00 for DATLAT calibration issue, */
		/* DQS input enable will refer to DATLAT */
		/* if need to enable this (for power saving), */
		/* do it after all calibration done */
		...
		/* DDR3 reset */
		udelay(200);
		ucstatus |= RegisterWriteVal(p, 0x0e4, 0x000000a3);
		udelay(500);
		...
		ucstatus |= RegisterWriteVal(p, 0x090, 0x00000000);
		ucstatus |= RegisterWriteVal(p, 0x094, 0x40404040);
		ucstatus |= RegisterWriteVal(p, 0x0dc, 0x00000000);
		ucstatus |= RegisterWriteVal(p, 0x0e0, 0x14000000);
		ucstatus |= RegisterWriteVal(p, 0x118, 0x00000000);
		ucstatus |= RegisterWriteVal(p, 0x0f0, 0x02000000);
		ucstatus |= RegisterWriteVal(p, 0x0f4, 0x11000000);
		/* sync mod (Rome will use sync mode only) */
		ucstatus |= RegisterWriteVal(p, 0x0fc, 0x17000000);
		ucstatus |= RegisterWriteVal(p, 0x168, 0x00000080);
		ucstatus |= RegisterWriteVal(p, 0x130, 0x30000000);
		udelay(1);
		ucstatus |= RegisterWriteVal(p, 0x004, 0xc0748481);
		ucstatus |= RegisterWriteVal(p, 0x124, 0xc0000011);
		ucstatus |= RegisterWriteVal(p, 0x1c0, 0x00000000);
		/* for DDR3, DISDMOEDIS [16] must be 1 */
		/* due to self refresh exit clock stable */
		/* timing (tCKSRX) issue */
		ucstatus |= RegisterWriteVal(p, 0x1ec, 0x00110001);
		ucstatus |= RegisterWriteVal(p, 0x07c, 0x00003301);
		ucstatus |= RegisterWriteVal(p, 0x080, 0x000008e0);
		ucstatus |= RegisterWriteVal(p, 0x028, 0xf1000000);
		ucstatus |= RegisterWriteVal(p, 0x158, 0x00000000);
		ucstatus |= RegisterWriteVal(p, 0x0e4, 0x000000a7);
		udelay(2);
		/* write ODT fixed to disable */
		ucstatus |= RegisterWriteVal(p, 0x054, 0x00000001);

		/* MR2 */
		ucstatus |= RegisterWriteVal(p, 0x088, 0x00004020);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000001);
		udelay(1);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000000);

		/* MR3 */
		ucstatus |= RegisterWriteVal(p, 0x088, 0x00006000);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000001);
		udelay(1);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000000);

		/* MR1 */
		ucstatus |= RegisterWriteVal(p, 0x088, 0x00002000);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000001);
		udelay(1);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000000);

		/* MR0 */
		ucstatus |= RegisterWriteVal(p, 0x088, 0x00001f15);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000001);
		udelay(1);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000000);

		/* ZQ calibration enable */
		ucstatus |= RegisterWriteVal(p, 0x088, 0x00000400);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000010);
		udelay(1);
		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00000000);
		udelay(1);

		ucstatus |= RegisterWriteVal(p, 0x1e4, 0x00001100);
		ucstatus |= RegisterWriteVal(p, 0x0e4, 0x000000a3);
		ucstatus |= RegisterWriteVal(p, 0x1dc, 0xd5643840);
		ucstatus |= RegisterWriteVal(p, 0x00c, 0x00000000);
		ucstatus |= RegisterWriteVal(p, 0x000, 0x558d48e7);
		/* disable DQSICALI_NEW [23] : HW RX window calibration */
		ucstatus |= RegisterWriteVal(p, 0x044, 0x9f3A0480);
		ucstatus |= RegisterWriteVal(p, 0x1e8, 0x80000060);
		ucstatus |= RegisterWriteVal(p, 0x008, 0x0000006c);
		ucstatus |= RegisterWriteVal(p, 0x010, 0x00000000);
		ucstatus |= RegisterWriteVal(p, 0x100, 0x00000000);
		ucstatus |= RegisterWriteVal(p, 0x01c, 0x12121212);
		ucstatus |= RegisterWriteVal(p, 0x0f8, 0x00000000);
...
void DramcEnterSelfRefresh(DRAMC_CTX_T *p, u8 op)
{
	...
	if (op == 1) { /* enter self refresh */
		...
		RegisterSetBit(p, DRAMC_REG_CONF1, POS_CONF1_SELFREF);
		udelay(2);
		...
		RegisterClrBit(p, DRAMC_REG_CONF1, POS_CONF1_SELFREF);
		udelay(2);
...
void DramcPhyReset(DRAMC_CTX_T *p)
{
	/* reset phy */
	/* 0x0f0[28] = 1 -> 0 */
	RegisterSetBit(p, DRAMC_REG_PHYCTL1, POS_PHYCTL1_PHYRST);

	/* read data counter reset */
	/* 0x0f4[25] = 1 -> 0 */
	RegisterSetBit(p, DRAMC_REG_GDDR3CTL1, POS_GDDR3CTL1_RDATRST);

	/* delay 10ns, need to change when porting */
	udelay(1);

	RegisterClrBit(p, DRAMC_REG_PHYCTL1, POS_PHYCTL1_PHYRST);
	RegisterClrBit(p, DRAMC_REG_GDDR3CTL1, POS_GDDR3CTL1_RDATRST);
}
...
void DramcRunTimeConfig(DRAMC_CTX_T *p)
{
	...
	/* HW_GATING defined */
	/* 0x1c0[31] */
	ucstatus |= RegisterSetBit(p, DRAMC_REG_DQSCAL0, POS_DQSCAL0_STBCALEN);
	...
	if (p->dram_type == TYPE_LPDDR3) {
		...
		if ((*ptr & 0x1) == 0x1) {
			...
			ucstatus |= RegisterSetBit(p, 0x1ec, 25);
		} else {
			/* single channel */
		 ...
		 u8 testaudpat, s16 loopforever, u8 period, u8 log2loopcount)
{
	...
	while (1) {
		...
		if (testaudpat == 2) { /* xtalk */
			...
			RegisterClrBit(p, DRAMC_REG_TEST2_4,
