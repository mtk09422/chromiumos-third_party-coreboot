...
dramc_ao_regs *ao_regs;
dramc_nao_regs *nao_regs;
dramc_ddrphy_regs *ddrphy_regs;

dramc_ao_regs *cha_ao_regs = (void *)CHA_DRAMCAO_BASE;
dramc_nao_regs *cha_nao_regs = (void *)CHA_DRAMCNAO_BASE;
dramc_ddrphy_regs *cha_ddrphy_regs = (void *)CHA_DDRPHY_BASE;

dramc_ao_regs *chb_ao_regs = (void *)CHB_DRAMCAO_BASE;
dramc_nao_regs *chb_nao_regs = (void *)CHB_DRAMCNAO_BASE;
dramc_ddrphy_regs *chb_ddrphy_regs = (void *)CHB_DDRPHY_BASE;
...
inline u32 DramcGetMR2ByFreq(int dram_freq)
{
	u32 value;
...
static inline u8 sel_ph_and_write_latency_setting(DRAMC_CTX_T *p)
{
	...
	if(p->channel == CHANNEL_A){
		...
		write32(&cha_ao_regs->lpddr2_3, 0x220fffff);
		write32(&cha_ddrphy_regs->lpddr2_3, 0x220fffff);
		write32(&cha_ddrphy_regs->lpddr2_4, 0x220fffff);

		write32(&cha_ddrphy_regs->selph12, 0x10f010f0);
		write32(&cha_ddrphy_regs->selph13, 0xffffffff);
		write32(&cha_ddrphy_regs->selph14, 0xffffffff);
		write32(&cha_ddrphy_regs->selph15, 0x0000001f);

	} else {

		write32(&chb_ao_regs->lpddr2_3, 0x220fffff);
		write32(&chb_ddrphy_regs->lpddr2_3, 0x220fffff);
		write32(&chb_ddrphy_regs->lpddr2_4, 0x220fffff);

		write32(&chb_ddrphy_regs->selph12, 0x10f010f0);
		write32(&chb_ddrphy_regs->selph13, 0xffffffff);
		write32(&chb_ddrphy_regs->selph14, 0xffffffff);
		write32(&chb_ddrphy_regs->selph15, 0x0000001f);
	}
...
DRAM_STATUS_T MemPllPreInit(DRAMC_CTX_T *p)
{
	...
	/* SPM_MODIFY */
	write32(&cha_ddrphy_regs->peri[2], 0x1111ff11);
	write32(&cha_ddrphy_regs->peri[3], 0x11511111);

	write32(&chb_ddrphy_regs->peri[2], 0x1111ff11);
	write32(&chb_ddrphy_regs->peri[3], 0x11511111);

	mcSHOW_DBG_MSG3("DataCheck=%p\n", &chb_ddrphy_regs->peri[3]);
	...
	if ((p->package == PACKAGE_POP) || (p->package == PACKAGE_SBS)) {
		...
		/* PLL: SPM MODIFY and SPM control clock enable */
		write32(&cha_ddrphy_regs->mempll_divider, 0x09008023);
		write32(&cha_ddrphy_regs->mempll05_divider, 0x09008004);

		write32(&chb_ddrphy_regs->mempll_divider, 0x09008023);
		write32(&chb_ddrphy_regs->mempll05_divider, 0x09008004);

		/* ALLCLK_EN */
		setbits_le32(&cha_ddrphy_regs->mempll_divider, 1 << 4);
		setbits_le32(&chb_ddrphy_regs->mempll_divider, 1 << 4);

		/* Disable gating function */
		clrbits_le32(&cha_ddrphy_regs->ddrphy_cg_ctrl, 1 << 2);
		clrbits_le32(&cha_ddrphy_regs->ddrphy_cg_ctrl, 1 << 1);

		clrbits_le32(&chb_ddrphy_regs->ddrphy_cg_ctrl, 1 << 2);
		clrbits_le32(&chb_ddrphy_regs->ddrphy_cg_ctrl, 1 << 1);

		/* PLL: SPM control clock enable */
		clrbits_le32(&cha_ddrphy_regs->mempll_divider, 1 << 15);
		clrbits_le32(&cha_ddrphy_regs->mempll_divider, 1 << 0);

		clrbits_le32(&chb_ddrphy_regs->mempll_divider, 1 << 15);
		clrbits_le32(&chb_ddrphy_regs->mempll_divider, 1 << 0);

		/* DRAMC 2X mode [0]FDIV2=1 */
		setbits_le32(&cha_ao_regs->ddr2ctl, 1 << 0);
		setbits_le32(&chb_ao_regs->ddr2ctl, 1 << 0);

		/* Delay counters for SPM power-up MEMPLL */
		write32(&cha_ddrphy_regs->peri[0], 0x21271b03);
		write32(&cha_ddrphy_regs->peri[1], 0x5096061e);

		write32(&chb_ddrphy_regs->peri[0], 0x21271b03);
		write32(&chb_ddrphy_regs->peri[1], 0x5096061e);

		/* [27] LDO_PHY2POP_NDIS, [24]LDO_PHY2POP_EN, */
		/* [7] MEMCLKENMODE, [3] MEMCLKEN_SEL, [0] MEMCLKENB_SEL */
		setbits_le32(&cha_ddrphy_regs->mempll_divider, 1 << 7);
		setbits_le32(&cha_ddrphy_regs->mempll_divider, 1 << 24);
		clrbits_le32(&cha_ddrphy_regs->mempll_divider, 1 << 3);
		/* must clear to select external source to get */
		/* correct phase for channel A */

		setbits_le32(&chb_ddrphy_regs->mempll_divider, 1 << 7);
		setbits_le32(&chb_ddrphy_regs->mempll_divider, 1 << 24);
	}
	return ucstatus;
}
...
DRAM_STATUS_T MemPllInit(DRAMC_CTX_T *p)
{
	...
	if ((p->package == PACKAGE_POP) || (p->package == PACKAGE_SBS)) {
		...
		/* == MEMPLL IP Part == */
		/* mempll config: set to seal ring */

		/* [29]RG_MEMPLL_REFCK_MODE_SEL -> 1: seal-ring, 0: cts */
		/* [28]RG_MEMPLL_REFCK_SEL=0 */
		/* [0]RG_MEMPLL_REFCK_EN=0 (only 0x688) */
		write32(&cha_ddrphy_regs->mempll[14], 0x00000000);
		write32(&chb_ddrphy_regs->mempll[14], 0x00000000);

		/* MEMPLL_BIAS_EN=0 : 0x60c[6]/0x65c[6] */
		/* MEMPLL_BIAS_LPF_EN=0 : 0x60c[7]/0x65c[7] */
		write32(&cha_ddrphy_regs->mempll[3], 0xd0000000);
		write32(&chb_ddrphy_regs->mempll[3], 0xd0000000);
		...
		write32(&cha_ddrphy_regs->mempll[5], 0x07800000 | pattern1);
		write32(&cha_ddrphy_regs->mempll[6], 0x4c00c000 | pattern2);
		write32(&cha_ddrphy_regs->mempll[7], 0x00020001 | pattern3);

		write32(&chb_ddrphy_regs->mempll[5], 0x07800000 | pattern1);
		write32(&chb_ddrphy_regs->mempll[6], 0x4c00c000 | pattern2);
		write32(&chb_ddrphy_regs->mempll[7], 0x00020001 | pattern3);

		/* mempll 4 */
		write32(&cha_ddrphy_regs->mempll[11], 0x07800000 | pattern1);
		write32(&cha_ddrphy_regs->mempll[12], 0x4c00c000 | pattern2);
		write32(&cha_ddrphy_regs->mempll[13], 0x00000001 | pattern3);

		write32(&chb_ddrphy_regs->mempll[11], 0x07800000 | pattern1);
		write32(&chb_ddrphy_regs->mempll[12], 0x4c00c000 | pattern2);
		write32(&chb_ddrphy_regs->mempll[13], 0x00000001 | pattern3);

		/* mempll 3 - enable signal tie together */
		/* sim error due to unknown dividor. */
		write32(&cha_ddrphy_regs->mempll[8], 0x07800000 | pattern1);
		write32(&cha_ddrphy_regs->mempll[9], 0x4c00c000 | pattern2);
		write32(&cha_ddrphy_regs->mempll[10], 0x00020001 | pattern3);

		write32(&chb_ddrphy_regs->mempll[8], 0x07800000 | pattern1);
		write32(&chb_ddrphy_regs->mempll[9], 0x4c00c000 | pattern2);
		write32(&chb_ddrphy_regs->mempll[10], 0x00020001 | pattern3);

		udelay(1); /* wait 100ns (after DA_MPLL_SDM_ISO_EN goes LOW) */

		/* Power up sequence starts: */
		/* MPLL_EN=1 RG_MPLL_EN=1: 0x10209280[0] */
		u4value = (*(volatile unsigned int *)(0x10209280));
		mcSET_BIT(&u4value, 0);
		(*(volatile unsigned int *)(0x10209280)) = u4value;
		...
		write32(&cha_ddrphy_regs->mempll[3], 0xd0000040);
		write32(&chb_ddrphy_regs->mempll[3], 0xd0000040);

		udelay(2); /* wait 2us */

		/* MEMPLL2_EN -> MEMPLL4_EN -> MEMPLL3_EN */
		setbits_le32(&cha_ddrphy_regs->mempll[5], 1 << 0);
		setbits_le32(&cha_ddrphy_regs->mempll[11], 1 << 0);
		setbits_le32(&cha_ddrphy_regs->mempll[8], 1 << 0);

		/* MEMPLL2_EN -> MEMPLL4_EN -> MEMPLL3_EN */
		setbits_le32(&chb_ddrphy_regs->mempll[5], 1 << 0);
		setbits_le32(&chb_ddrphy_regs->mempll[11], 1 << 0);
		setbits_le32(&chb_ddrphy_regs->mempll[8], 1 << 0);

		udelay(100); /* wait 100us */

		/* MEMPLL_BIAS_LPF_EN=1 : 0x60c[7] */
		setbits_le32(&cha_ddrphy_regs->mempll[3], 7);
		setbits_le32(&chb_ddrphy_regs->mempll[3], 7);

		udelay(30); /* wait 20us or 30us */
		...
		mcSHOW_DBG_MSG3("VCOK_05PHY_Mempll2=%#x, ", VCOK_05PHY_Mempll2);
		...
		clrbits_le32(&cha_ddrphy_regs->mempll[5], 1 << 0);
		clrbits_le32(&cha_ddrphy_regs->mempll[11], 1 << 0);
		clrbits_le32(&cha_ddrphy_regs->mempll[8], 1 << 0);

		clrbits_le32(&chb_ddrphy_regs->mempll[5], 1 << 0);
		clrbits_le32(&chb_ddrphy_regs->mempll[11], 1 << 0);
		clrbits_le32(&chb_ddrphy_regs->mempll[8], 1 << 0);

		/* MEMPLL*_AUTOK_EN=0 */
		/* Disable AUTOK MEMPLL2_EN -> MEMPLL4_EN -> MEMPLL3_EN */
		clrbits_le32(&cha_ddrphy_regs->mempll[5], 1 << 23);
		clrbits_le32(&cha_ddrphy_regs->mempll[11], 1 << 23);
		clrbits_le32(&cha_ddrphy_regs->mempll[8], 1 << 23);

		clrbits_le32(&chb_ddrphy_regs->mempll[5], 1 << 23);
		clrbits_le32(&chb_ddrphy_regs->mempll[11], 1 << 23);
		clrbits_le32(&chb_ddrphy_regs->mempll[8], 1 << 23);

		udelay(1); /* wait 1us */

		/* MEMPLL[2->4->3]_FB_MCK_SEL=1 (switch to outer loop) */
		setbits_le32(&cha_ddrphy_regs->mempll[6], 1 << 25);
		setbits_le32(&cha_ddrphy_regs->mempll[12], 1 << 25);
		setbits_le32(&cha_ddrphy_regs->mempll[9], 1 << 25);

		setbits_le32(&chb_ddrphy_regs->mempll[6], 1 << 25);
		setbits_le32(&chb_ddrphy_regs->mempll[12], 1 << 25);
		setbits_le32(&chb_ddrphy_regs->mempll[9], 1 << 25);
		...
		*(volatile unsigned int *)(0x10006010) &= ~(0x08000000);
		...
		udelay(1); /* PLL ready, wait 1us */

		write32(&cha_ddrphy_regs->mempll_divider, 0x090000B1);
		/* SPM control clock enable */
		write32(&cha_ddrphy_regs->mempll_divider, 0x090000B2);
		write32(&cha_ddrphy_regs->mempll_divider, 0x090000B1);

		write32(&chb_ddrphy_regs->mempll_divider, 0x090000B1);
		/* SPM control clock enable */
		write32(&chb_ddrphy_regs->mempll_divider, 0x090000B2);
		write32(&chb_ddrphy_regs->mempll_divider, 0x090000B1);
