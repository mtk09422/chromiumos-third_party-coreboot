...
struct it6151_bridge {
	struct mtk_i2c_t it6151_mipirx;
	struct mtk_i2c_t it6151_dptx;
	...
	u8 dev_address, u8 reg_off, u8 *data)
{
	...
	*data = reg_off;
	...
	if (dev_address == ite_bridge->it6151_mipirx.addr) {
		ret = mtk_i2c_write_read(&ite_bridge->it6151_mipirx,
					 data, len, len);
	} else if (dev_address == ite_bridge->it6151_dptx.addr) {
		ret = mtk_i2c_write_read(&ite_bridge->it6151_dptx,
					 data, len, len);
	}
	...
	u8 dev_address, u8 reg_address, u8 reg_val)
{
	...
	if (dev_address == ite_bridge->it6151_mipirx.addr)
		mtk_i2c_write(&ite_bridge->it6151_mipirx, write_data, 2);
	else if (dev_address == ite_bridge->it6151_dptx.addr)
		mtk_i2c_write(&ite_bridge->it6151_dptx, write_data, 2);
}
...
{
	mt_set_gpio_mode(pin, GPIO_MODE_00); /* 0:GPIO mode */
	mt_set_gpio_dir(pin, GPIO_DIR_OUT); /* dir = output */
...
void it6151_enable(void)
{
	...
	pmic_config_interface(DIGLDO_CON6, 0x1, PMIC_RG_VCAMIO_SW_EN_MASK,
...
{
	_ite_bridge.it6151_mipirx.base = 0x11010000;
	_ite_bridge.it6151_mipirx.id = 3;
	_ite_bridge.it6151_mipirx.dir = 0;
	_ite_bridge.it6151_mipirx.addr = MIPI_I2C_ADDR;
	_ite_bridge.it6151_mipirx.mode = ST_MODE;
	_ite_bridge.it6151_mipirx.speed = 100;

	_ite_bridge.it6151_dptx.base = 0x11010000;
	_ite_bridge.it6151_dptx.id = 3;
	_ite_bridge.it6151_dptx.dir = 0;
	_ite_bridge.it6151_dptx.addr = DP_I2C_ADDR;
	_ite_bridge.it6151_dptx.mode = ST_MODE;
	_ite_bridge.it6151_dptx.speed = 100;
	_ite_bridge.gpio_pd_n  = (GPIO93 | 0x80000000);
	_ite_bridge.gpio_rst_n = (GPIO94 | 0x80000000);
