	...
	u8 dev_address, u8 reg_off, u8 *data)
{
	...
	if (dev_address == MIPI_I2C_ADDR) {
		ret = i2c_readb(3, MIPI_I2C_ADDR, reg_off, data);
	} else if (dev_address == DP_I2C_ADDR) {
		ret = i2c_readb(3, DP_I2C_ADDR, reg_off, data);
	}
...
{
	if (dev_address == MIPI_I2C_ADDR)
		i2c_writeb(3, MIPI_I2C_ADDR, reg_address, reg_val);
	else if (dev_address == DP_I2C_ADDR)
		i2c_writeb(3, DP_I2C_ADDR, reg_address, reg_val);
}
