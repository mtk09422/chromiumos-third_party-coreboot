...
#define efuse_is_io_33v()		(((*EFUSE_HW2_RES0) & EFUSE_HW2_RES0_IO_33V) ? 0 : 1)
...
static void sf_nor_gpio_init(void)
{
	...
	clrsetbits_le16((void*)GPIO_DRVSEL(2),  DRVS_BIT(4), DRVS_DAT(4, 0x2));
	clrsetbits_le16((void*)GPIO_DRVSEL(3),  DRVS_BIT(0), DRVS_DAT(0, 0x2));
	clrsetbits_le16((void*)GPIO_PULLSEL(1),  PULL_BIT(4) | PULL_BIT(5) |
	 		PULL_BIT(6) | PULL_BIT(7) | PULL_BIT(8) | PULL_BIT(9),
		    PULL_DAT(4, 0x1) | PULL_DAT(5, 0x1) | PULL_DAT(6, 0x1) |
		    PULL_DAT(7, 0x1) | PULL_DAT(8, 0x1) | PULL_DAT(9, 0x1) );

	clrsetbits_le16((void*)EXMD_CTRL, EXMD_TDSEL, 0xa);
	if (efuse_is_io_33v())
		clrsetbits_le16((void*)EXMD_CTRL, EXMD_RDSEL, 0x0c0);

	if (efuse_is_io_33v())
		clrsetbits_le16((void*)EXMD_CTRL, EXMD_BIAS, 0x5000);
	else
		printk(BIOS_DEBUG, "IO isnot 3.3V !\n");

	clrsetbits_le16((void*)GPIO_MODE(1), MODE_BIT(4),
		       MODE_DAT(4, GPIO_MODE6) );
	clrsetbits_le16((void*)GPIO_MODE(2), MODE_BIT(0) | MODE_BIT(1) |
		MODE_BIT(2) | MODE_BIT(3) | MODE_BIT(4),
		MODE_DAT(0, GPIO_MODE6) |MODE_DAT(1, GPIO_MODE6) |
		MODE_DAT(2, GPIO_MODE6) |MODE_DAT(3, GPIO_MODE6) |
		       MODE_DAT(4, GPIO_MODE6) );
}
...
static int sflashhw_get_set_id(uint32_t index, uint8_t *manufacture_id,
							   uint8_t *device_id_1,uint8_t *device_id_2)
{
	write8(&mt8173_nor->sflash_prgdata_reg[5], 0x90);
	write8(&mt8173_nor->sflash_prgdata_reg[4], 0x00);
	write8(&mt8173_nor->sflash_prgdata_reg[3], 0x00);
	write8(&mt8173_nor->sflash_prgdata_reg[2], 0x00);
	write8(&mt8173_nor->sflash_prgdata_reg[1], 0x00);
	write8(&mt8173_nor->sflash_prgdata_reg[0], 0x00);
	...
	if (device_id_2 != 0)
		*device_id_2 = 0x10;
	if (device_id_1 != 0)
		*device_id_1 = read8(&mt8173_nor->sflash_shreg_reg[0]);
	if (manufacture_id != 0)
		*manufacture_id = read8(&mt8173_nor->sflash_shreg_reg[1]);
	send_flash_command(0x0);
	return 0;
}

static int sflashhw_getid(uint32_t index, uint8_t *manufacture_id,
	                      uint8_t *device_id_1, uint8_t *device_id_2)
{
	write8(&mt8173_nor->sflash_prgdata_reg[5], 0x9F);
	write8(&mt8173_nor->sflash_prgdata_reg[4], 0x00);
	write8(&mt8173_nor->sflash_prgdata_reg[3], 0x00);
	write8(&mt8173_nor->sflash_prgdata_reg[2], 0x00);
	...
	if (device_id_2 != 0)
		*device_id_2 = read8(&mt8173_nor->sflash_shreg_reg[0]);
	if (device_id_1 != 0)
		*device_id_1 = read8(&mt8173_nor->sflash_shreg_reg[1]);
	if (manufacture_id != 0)
		*manufacture_id = read8(&mt8173_nor->sflash_shreg_reg[2]);
	send_flash_command(0x0);
	if ((manufacture_id != 0) && (device_id_1 != 0)) {
		if ((*manufacture_id == 0xFF) || (*device_id_1 == 0x00)) {
			printk(BIOS_DEBUG, "Try not JEDEC ID\n");
			if (sflashhw_get_set_id(index, manufacture_id, device_id_1,
					 device_id_2) != 0)
				return 1;
		}
	}
...
static int do_identify(uint32_t index, uint32_t *vendor)
{
	...
	*vendor = 0xFFFFFFFF;
	...
	printk(BIOS_DEBUG, "index :%d, id: 0x%X, ID1: 0x%X, ID2: 0x%X\n",
...
static int wb_enable(void)
{
	...
	write8(&mt8173_nor->sflash_cfg_reg[1], temp);
	polling = 0;
	while (1) {
		reg = read8(&mt8173_nor->sflash_cfg_reg[1]);
		if (0x01 == (reg & 0x1))
			break;
		polling++;
		if (polling > SFLASH_POLLINGREG_COUNTER)
...
{
	write8(&mt8173_nor->sflash_prgdata_reg[5],
		 array_flash_chip[index].wren_cmd);
...
static void sflashhw_get_flash_info(SFLASH_INFO_T *pr_info)
{
	...
	for (i = 0; i < chip_counter; i++) {
		...
		memcpy(pr_info->ar_flash_info[i].pc_flash_str,
		 array_flash_chip[i].pc_flash_str, SFLASHNAME_LENGTH);
	}
}
...
static int sflashhw_read(uint32_t addr, uint32_t length, u8 *buffer)
{
	...
	write8(&mt8173_nor->sflash_cfg_reg[1], 0x0C);
	write8(&mt8173_nor->sflash_read_dual_reg,
		 read8(&mt8173_nor->sflash_read_dual_reg) & 0x10);
	{
		write8(&mt8173_nor->sflash_read_dual_reg,
			 read8(&mt8173_nor->sflash_read_dual_reg) | 0x0);
	}
	write8(&mt8173_nor->sflash_radr_reg[2], LoByte(HiWord(addr)));
	write8(&mt8173_nor->sflash_radr_reg[1], HiByte(LoWord(addr)));
	write8(&mt8173_nor->sflash_radr_reg[0], LoByte(LoWord(addr)));
...
static int execute_write_cmd(uint32_t index)
{
	...
	if (sf_isr_en) {
		sflash_isr_status = 0;
		if (aai_write == 0) {
			write32(&mt8173_nor->sflash_sf_intrstus_reg, 0x10);
			write32(&mt8173_nor->sflash_sf_intren_reg, 0x10);
		}else {
			write32(&mt8173_nor->sflash_sf_intrstus_reg, 0x40);
	...
	if (sf_isr_en) {
		while (((aai_write == 0) && (!(sflash_isr_status & 0x10))) ||
		 ((aai_write == 1) && (!(sflash_isr_status & 0x40)))) {
		}
	}else {
		while (1) {
			if (aai_write == 0) {
	...
	if (sf_isr_en)
		write32(&mt8173_nor->sflash_sf_intren_reg, 0x0);
	send_flash_command(0x0);
	return 0;
}

static int write_buffer(uint32_t index, uint32_t addr,
...
static int write_single_byte(uint32_t index, uint32_t addr,
			     uint8_t data)
{
	write8(&mt8173_nor->sflash_radr_reg[2], LoByte(HiWord(addr)));
	write8(&mt8173_nor->sflash_radr_reg[1], HiByte(LoWord(addr)));
...
static int sflashhw_write_sector(uint32_t index, uint32_t addr,
				 uint32_t length, const u8 *buffer)
{
	uint32_t i, counter, page_align;

	if (index >= MAX_FLASH_COUNTER) {
		printk(BIOS_DEBUG, "Nor index is out fo Max counter\n");
		return 1;
	}
	...
	if (array_flash_chip[index].pp_type == 0) {
		while ((int)length > 0) {
			...
			if (write_buffer(index, addr, counter, buffer) != 0) {
				printk(BIOS_DEBUG, "error! faddr = 0x%x, length = 0x%x\n",
					 addr, counter);
				if (wb_disable() != 0)
					return 1;
...
{
	printk(BIOS_DEBUG, "..cdd write_sf_protect val = 0x%x!\n", val);
...
static int sflashhw_write_protect(uint32_t index,
	 unsigned char enable)
{
	...
	if (write_protect) {
		...
		if (enable)
			write8(&mt8173_nor->sflash_prgdata_reg[5], 0x3C);
		else{
			if (nor_partial_protection)
				write32(&mt8173_nor->sflash_prgdata_reg[5],
				 array_flash_chip[index].protection);
			else
				write32(&mt8173_nor->sflash_prgdata_reg[5], 0x0);
		}
		write8(&mt8173_nor->sflash_cnt_reg, 8);
...
static int sflashhw_init(void)
{
	...
	sflashhw_write_sf_protect(0x30);
	memset((void *)array_flash_chip, 0x0,
		 MAX_FLASH_COUNTER * sizeof(SFLASHHW_VENDOR_T));
	sf_nor_gpio_init();
	...
	for (loop = 0; loop < MAX_FLASH_COUNTER; loop++) {
		if (do_identify(loop, &VendorIdx) != 0)
			continue;
		memcpy((void *)&(array_flash_chip[chip_counter]),
		 (void *)&(array_vendor_flash[VendorIdx]),
...
static int sflash_write(uint32_t offset, uint64_t mem_ptr,
	 uint32_t byte_counter)
{
	uint8_t *buffer;
	uint32_t i, j, write_addr, length, write_byte;
	uint32_t chip_offset, sect_start, sect_end;
...
static int sflash_read(uint32_t offset, uint64_t mem_ptr,
					   uint32_t byte_counter)
{
...
static int nor_write(u32 offset, u32 mem_ptr, u32 byte_counter)
{
	...
	do {
		...
		ret = sflash_write((u32)(offset + write_offset),
			 (uint64_t)(mem_ptr + write_offset), write_cnt);
		write_offset += write_cnt;
		byte_counter -= write_cnt;
	} while (byte_counter);
	return ret;
}
static int nor_bread(struct blkdev * bdev, u32 blknr, u32 blks,
	 u8 * buf, u32 part_id)
{
	u32 ret;
	...
	if ((ret = nor_read((unsigned int )offset,
		 (unsigned int)(uintptr_t)buf, byte_length)) != 0 )
		printk(BIOS_DEBUG, "partitial Read test fail !!! \n");

	return 0;
}

static int nor_bwrite(struct blkdev * bdev, u32 blknr,
	 u32 blks, u8 * buf, u32 part_id)
{
	u32 ret;
	u32 offset = blknr * bdev->blksz;
