...
static s32 pwrap_init_cipher(void);
static s32 pwrap_init_sidly(void);
static s32 pwrap_init_reg_clock(u32 regck_sel);
...
s32 pwrap_wacs2(u32 write, u32 adr, u32 wdata, u32 *rdata, u32 init_check)
{
	u32 reg_rdata = 0;
	u32 wacs_write = 0;
	u32 wacs_adr = 0;
	...
	if (init_check) {
		reg_rdata = read32(&mt8173_pmic_pwrap->wacs2_rdata);
	...
	return_value = wait_for_state_idle(TIMEOUT_WAIT_IDLE,
					   &mt8173_pmic_pwrap->wacs2_rdata,
					   &mt8173_pmic_pwrap->wacs2_vldclr,
					   0);
	if (return_value != 0) {
	...
	write32(&mt8173_pmic_pwrap->wacs2_cmd, wacs_cmd);
	if (write == 0) {
		if (NULL == rdata) {
		...
		*rdata = ((reg_rdata >> RDATA_WACS_RDATA_SHIFT)
		...
		write32(&mt8173_pmic_pwrap->wacs2_vldclr, 1);
	}
FAIL:
	return return_value;
}
...
static s32 pwrap_init_dio(u32 dio_en)
{
	...
	wait_for_state_ready(wait_for_idle_and_sync,
				  ...
				  0);
	if (return_value != 0) {
		pwrap_err("pwrap_init_dio fail,return_value=%#x\n",
			 return_value);
		return return_value;
	}
	write32(&mt8173_pmic_pwrap->dio_en, dio_en);
	/* Read Test */
	pwrap_read_nochk(DEW_READ_TEST, &rdata);
	if (rdata != DEFAULT_VALUE_READ_TEST) {
		pwrap_err("fail,dio_en = %#x, READ_TEST rdata=%#x, exp=0x5aa5\n",
			 dio_en, rdata);
...
static s32 pwrap_init_cipher(void)
{
	...
	write32(&mt8173_pmic_pwrap->cipher_swrst, 1);
	write32(&mt8173_pmic_pwrap->cipher_swrst, 0);
	write32(&mt8173_pmic_pwrap->cipher_key_sel, 1);
	write32(&mt8173_pmic_pwrap->cipher_iv_sel, 2);
	write32(&mt8173_pmic_pwrap->cipher_en, 1);
	...
	return_value =
	wait_for_state_ready(wait_for_cipher_ready, TIMEOUT_WAIT_IDLE,
				  &mt8173_pmic_pwrap->cipher_rdy, 0);
	if (return_value != 0) {
		pwrap_err("wait for cipher ready@AP fail,return_value=%#x\n",
			  return_value);
		return return_value;
	}
	...
	do {
		...
		if (stopwatch_expired(&sw)) {
			pwrap_err("wait for cipher data ready timeout\n");
			return E_PWR_WAIT_IDLE_TIMEOUT;
		}
	...
	return_value =
		wait_for_state_ready(wait_for_idle_and_sync,
					  TIMEOUT_WAIT_IDLE,
					  &mt8173_pmic_pwrap->wacs2_rdata, 0);
	if (return_value != 0) {
		pwrap_err("wait for cipher mode idle fail,return_value=%#x\n",
			 return_value);
		return return_value;
	}
	write32(&mt8173_pmic_pwrap->cipher_mode, 1);

	/* Read Test */
	pwrap_read_nochk(DEW_READ_TEST, &rdata);
	if (rdata != DEFAULT_VALUE_READ_TEST) {
		pwrap_err("pwrap_init_cipher,read test error,error code=%#x, rdata=%#x\n",
			 1, rdata);
		return E_PWR_READ_TEST_FAIL;
	}
...
static s32 pwrap_init_sidly(void)
{
	...
	for (i = 0; i < 4; i++) {
		write32(&mt8173_pmic_pwrap->sidly, i);
		pwrap_wacs2(0, DEW_READ_TEST, 0, &rdata, 0);
		if (rdata == DEFAULT_VALUE_READ_TEST) {
			pwrap_log("[Read Test] pass, SIDLY=%#x\n", i);
			pass |= 1 << i;
		}
	}

	if (dly[pass] < 0) {
		pwrap_err("sidly pass range 0x%#x not continuous\n",
				pass);
		return dly[pass];
	}

	write32(&mt8173_pmic_pwrap->sidly, dly[pass]);
...
static s32 pwrap_reset_spislv(void)
{
	...
	write32(&mt8173_pmic_pwrap->hiprio_arb_en, 0);
	write32(&mt8173_pmic_pwrap->wrap_en, 0);
	write32(&mt8173_pmic_pwrap->mux_sel, 1);
	write32(&mt8173_pmic_pwrap->man_en, 1);
	write32(&mt8173_pmic_pwrap->dio_en, 0);

	write32(&mt8173_pmic_pwrap->man_cmd, (OP_WR << 13) | (OP_CSL << 8));
	/* to reset counter */
	write32(&mt8173_pmic_pwrap->man_cmd, (OP_WR << 13) | (OP_OUTS << 8));
	write32(&mt8173_pmic_pwrap->man_cmd, (OP_WR << 13) | (OP_CSH << 8));
	/*
	...
	*/
	...
	return_value =
		wait_for_state_ready(wait_for_sync,
					  TIMEOUT_WAIT_IDLE,
					  &mt8173_pmic_pwrap->wacs2_rdata, 0);
	if (return_value != 0) {
		pwrap_err("pwrap_reset_spislv fail,return_value=%#x\n",
			 return_value);
		ret = E_PWR_TIMEOUT;
	}

	write32(&mt8173_pmic_pwrap->man_en, 0);
	write32(&mt8173_pmic_pwrap->mux_sel, 0);
...
static s32 pwrap_init_reg_clock(u32 regck_sel)
{
	...
	if (regck_sel == REG_CLOCK_18MHZ) {
		write32(&mt8173_pmic_pwrap->rddmy, 0xc);
		write32(&mt8173_pmic_pwrap->cshext_write, 0x0);
		write32(&mt8173_pmic_pwrap->cshext_read, 0x4);
		write32(&mt8173_pmic_pwrap->cslext_start, 0x0);
		write32(&mt8173_pmic_pwrap->cslext_end, 0x4);
	} else if (regck_sel == REG_CLOCK_26MHZ) {
		write32(&mt8173_pmic_pwrap->rddmy, 0xc);
		write32(&mt8173_pmic_pwrap->cshext_write, 0x0);
		write32(&mt8173_pmic_pwrap->cshext_read, 0x4);
		write32(&mt8173_pmic_pwrap->cslext_start, 0x2);
		write32(&mt8173_pmic_pwrap->cslext_end, 0x2);
	} else {
		write32(&mt8173_pmic_pwrap->rddmy, 0xf);
		write32(&mt8173_pmic_pwrap->cshext_write, 0xf);
		write32(&mt8173_pmic_pwrap->cshext_read, 0xf);
		write32(&mt8173_pmic_pwrap->cslext_start, 0xf);
		write32(&mt8173_pmic_pwrap->cslext_end, 0xf);
	}
	return 0;
}
...
s32 pwrap_init(void)
{
	...
	setbits_le32((void *)(uintptr_t)INFRA_GLOBALCON_RST0, 1 << 7);
	/* clear reset bit */
	clrbits_le32((void *)(uintptr_t)INFRA_GLOBALCON_RST0, 1 << 7);

	/* Enable DCM */
	write32(&mt8173_pmic_pwrap->dcm_en, 3);
	write32(&mt8173_pmic_pwrap->dcm_dbc_prd, 0);
	...
	/* Enable WACS2 */
	write32(&mt8173_pmic_pwrap->wrap_en, 1);
	write32(&mt8173_pmic_pwrap->hiprio_arb_en, WACS2);
	write32(&mt8173_pmic_pwrap->wacs2_en, 1);
	...
	/* Enable Encryption for security purpose*/
	sub_return = pwrap_init_cipher();
	if (sub_return != 0) {
		pwrap_err("Enable Encryption fail, return=%#x\n", sub_return);
	...
	write32(&mt8173_pmic_pwrap->crc_en, 0x1);
	write32(&mt8173_pmic_pwrap->sig_mode, 0x0);
	write32(&mt8173_pmic_pwrap->sig_adr, DEW_CRC_VAL);

	/* PMIC_WRAP enables */
	write32(&mt8173_pmic_pwrap->hiprio_arb_en, 0x1ff);
	write32(&mt8173_pmic_pwrap->wacs0_en, 0x1);
	write32(&mt8173_pmic_pwrap->wacs1_en, 0x1);
	/* wacs2_en already enabled */
	/* 0x1:20us,for concurrence test,MP:0x5; 100us */
	write32(&mt8173_pmic_pwrap->staupd_prd, 0x5);
	write32(&mt8173_pmic_pwrap->staupd_grpen, 0xf1);
	/*
	 * watch dog@pwrap,
	 * asserted an interrupt if some communication when pwrap work
	 */
	write32(&mt8173_pmic_pwrap->wdt_unit, 0xf);
	write32(&mt8173_pmic_pwrap->wdt_src_en, 0xffffffff);
	write32(&mt8173_pmic_pwrap->timer_en, 0x1);
	/* except for [31] debug_int */
	write32(&mt8173_pmic_pwrap->int_en, 0x7ffffffd);
	...
	if ((sub_return != 0) || (sub_return1 != 0)) {
		pwrap_err("enable dewrap fail,sub_return=%d,sub_return1=%d\n",
			  sub_return, sub_return1);
		return E_PWR_INIT_ENABLE_DEWRAP;
	}

	/* Initialization Done */
	write32(&mt8173_pmic_pwrap->init_done2, 0x1);
	write32(&mt8173_pmic_pwrap->init_done0, 0x1);
	write32(&mt8173_pmic_pwrap->init_done1, 0x1);
...
s32 pwrap_init_preloader(void)
{
	...
	if (pwrap_ret != 0)
		pwrap_err("pwrap_init failed, ret = %#x\n", pwrap_ret);
