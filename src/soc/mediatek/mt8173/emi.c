...
static void DRAM_Vcore_adjust(int nAdjust)
{
	switch (nAdjust) {
	case 1:
		mt6391_write(VCORE_CON9, Vcore_HV_LPPDR3, 0x7F, 0);
		mt6391_write(VCORE_CON10, Vcore_HV_LPPDR3, 0x7F, 0);
		break;
	case 2:
		mt6391_write(VCORE_CON9, Vcore_NV_LPPDR3, 0x7F, 0);
		mt6391_write(VCORE_CON10, Vcore_NV_LPPDR3, 0x7F, 0);
		break;
	case 3:
		mt6391_write(VCORE_CON9, Vcore_LV_LPPDR3, 0x7F, 0);
		mt6391_write(VCORE_CON10, Vcore_LV_LPPDR3, 0x7F, 0);
		break;
	default:
		mt6391_write(VCORE_CON9, Vcore_NV_LPPDR3, 0x7F, 0);
		mt6391_write(VCORE_CON10, Vcore_NV_LPPDR3, 0x7F, 0);
...
static void DRAM_Vmem_adjust(int nAdjust)
{
	switch (nAdjust) {
	case 1:
		mt6391_write(VDRM_CON9, Vmem_HV_LPDDR3, 0x7F, 0);
		mt6391_write(VDRM_CON10, Vmem_HV_LPDDR3, 0x7F, 0);
		break;
	case 2:
		mt6391_write(VDRM_CON9, Vmem_NV_LPDDR3, 0x7F, 0);
		mt6391_write(VDRM_CON10, Vmem_NV_LPDDR3, 0x7F, 0);
		break;
	case 3:
		mt6391_write(VDRM_CON9, Vmem_LV_LPDDR3, 0x7F, 0);
		mt6391_write(VDRM_CON10, Vmem_LV_LPDDR3, 0x7F, 0);
		break;
	default:
		mt6391_write(VDRM_CON9, Vmem_NV_LPDDR3, 0x7F, 0);
		mt6391_write(VDRM_CON10, Vmem_NV_LPDDR3, 0x7F, 0);
...
void dump_dramc_register(void)
{
        u32 i = 0;
        uintptr_t reg_addr = 0;
        u32 reg_value = 0;

        mcSHOW_DBG_MSG("dump dramc register\n");

        for (i = 0; i <= 0x450; i+=4 ) {
                reg_addr = (CHA_DRAMCAO_BASE + i);
		reg_value = *(volatile unsigned int *)(reg_addr);
                mcSHOW_DBG_MSG("a ao 0x%x=0x%x\n", i, reg_value);
        }
        for (i = 0; i <= 0x400; i+=4 ) {
                reg_addr = (CHA_DRAMCNAO_BASE + i);
		reg_value = *(volatile unsigned int *)(reg_addr);
                mcSHOW_DBG_MSG("a nao 0x%x=0x%x\n", i, reg_value);
        }
        for (i = 0; i <= 0x690; i+=4 ) {
                reg_addr = (CHA_DDRPHY_BASE + i);
		reg_value = *(volatile unsigned int *)(reg_addr);
                mcSHOW_DBG_MSG("a phy 0x%x=0x%x\n", i, reg_value);
        }
        for (i = 0; i <= 0x450; i+=4 ) {
                reg_addr = (CHB_DRAMCAO_BASE + i);
		reg_value = *(volatile unsigned int *)(reg_addr);
                mcSHOW_DBG_MSG("b ao 0x%x=0x%x\n", i, reg_value);
        }
        for (i = 0; i <= 0x400; i+=4 ) {
                reg_addr = (CHB_DRAMCNAO_BASE + i);
		reg_value = *(volatile unsigned int *)(reg_addr);
                mcSHOW_DBG_MSG("b nao 0x%x=0x%x\n", i, reg_value);
        }
        for (i = 0; i <= 0x690; i+=4 ) {
                reg_addr = (CHB_DDRPHY_BASE + i);
		reg_value = *(volatile unsigned int *)(reg_addr);
                mcSHOW_DBG_MSG("b phy 0x%x=0x%x\n", i, reg_value);
        }
}
...
static int mt_get_mdl_number(void)
{
	...
	if (MDL_first_search_done == 0) {
		...
		/* ram code 0 for hynix one rank */
		/* ram code 1 for samsung dual ranks */
		dram_rank_nr = (ram_code()==0)? 1: 2;

		/* read back value is not "0" means rank is there */
		printk(BIOS_DEBUG, "ram code of the board %d\n", ram_code());
