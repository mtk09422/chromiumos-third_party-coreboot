...
static struct mtk_i2c i2c[7] = {
	...
	{
		.i2c_addr = 0x11007000,
		.i2c_dma_addr = 0x11000100,
	},

	/* i2c1 setting */
	{
		.i2c_addr = 0x11008000,
		.i2c_dma_addr = 0x11000180,
	},

	/* i2c2 setting */
	{
		.i2c_addr = 0x11009000,
		.i2c_dma_addr = 0x11000200,
	},

	/* i2c3 setting */
	{
		.i2c_addr = 0x11010000,
		.i2c_dma_addr = 0x11000280,
	},

	 /* i2c4 setting */
	{
		.i2c_addr = 0x11011000,
		.i2c_dma_addr = 0x11000300,
	},
...
static uint32_t mtk_i2c_transfer(uint8_t bus, struct i2c_seg *seg, uint8_t read)
{
	...
	if (read == I2C_WRITE_MODE) {
		...
		if ((write_len == 0) || (write_len > 255)) {
			I2CERR("[i2c_transfer] I2C doesn't support len = %d.\n", write_len);
	...
	if (read == I2C_WRITE_MODE) {
		memcpy(_dma, write_buffer, write_len);
		...
		write32(I2CADDR_W(i2c_dma_addr, OFFSET_RX_MEM_ADDR), (uintptr_t)_dma);
	...
	while (1) {
		...
		if (status & I2C_HS_NACKERR) {
			...
			ret_code = I2C_OK;
			memcpy(read_buffer, _dma, read_len);
			break;
