...
s32 pwrap_init(void)
{
	...
	WRAP_WR32(WDT_SWSYSRST,
		  (0x88000000 | ((rdata | (0x1 << 11)) & 0x00ffffff)));
	...
	sub_return = _pwrap_init_reg_clock(2); /* 0:safe mode, 1:18MHz, 2:36MHz */
	if (sub_return != 0) {
	 ...
	 * NEW@MT6397
	 */
	sub_return = pwrap_write_nochk(PMIC_TOP_CKCON3, 0x0000);
	if (sub_return != 0) {
		PWRAPERR("switch event pin fail,sub_return=%d\n", sub_return);
		return E_PWR_INIT_ENABLE_EVENT;
	}
	/* Initialization Done */
