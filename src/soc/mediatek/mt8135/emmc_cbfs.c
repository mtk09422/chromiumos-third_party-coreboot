				 ...
				 size_t offset, size_t count)
{
	...
	DEBUG_EMMC("%s at offset: 0x%x, count: 0x%x\n", __func__, offset,
		   count);

	/* read unaligned data in 1st block */
	offset_within_block = offset % EMMC_BLOCK_SIZE;
	count_tmp = EMMC_BLOCK_SIZE - offset_within_block;
	emmc->bdev->bread(emmc->bdev, offset / EMMC_BLOCK_SIZE, 1, buffer);
	DEBUG_EMMC("%s 1 at block #: 0x%x, blocks count: 0x%x\n", __func__,
	...
	if ((offset < BOOT0_SIZE) && ((offset + count) > BOOT0_SIZE)) {
		...
		emmc->bdev->bread(emmc->bdev, offset / EMMC_BLOCK_SIZE,
				  count_tmp / EMMC_BLOCK_SIZE, dest);
		DEBUG_EMMC("%s 2 at block #: 0x%x, blocks count: 0x%x\n",
	...
	if (count / EMMC_BLOCK_SIZE) {
		emmc->bdev->bread(emmc->bdev, offset / EMMC_BLOCK_SIZE,
				  count / EMMC_BLOCK_SIZE, dest);
		DEBUG_EMMC("%s 3 at block #: 0x%x, blocks count: 0x%x\n",
			   __func__, offset / EMMC_BLOCK_SIZE,
	...
	if (count % EMMC_BLOCK_SIZE) {
		emmc->bdev->bread(emmc->bdev, offset / EMMC_BLOCK_SIZE, 1,
				  buffer);
		DEBUG_EMMC("%s 4 at block #: 0x%x, blocks count: 0x%x\n",
...
#else
	context.buffer.buffer = (void *)0x80180000;
