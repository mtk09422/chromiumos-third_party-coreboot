...
#ifndef SOC_MEDIATEK_MT8135_MMC_CORE_H
#define SOC_MEDIATEK_MT8135_MMC_CORE_H
...
struct mmc_csd {
	...
	unsigned int read_partial:1,
	    ...
	    write_misalign:1,
	    write_prot_grp:1,
	    perm_wr_prot:1, tmp_wr_prot:1, erase_blk_en:1, copy:1, dsr:1;
};
...
struct mmc_ext_csd {
	...
	unsigned char rev;
	unsigned char boot_info;
	unsigned char part_en:1, enh_attr_en:1, ddr_support:1;
...
struct sdio_cccr {
	...
	unsigned int multi_block:1,
	    low_speed:1,
	    wide_bus:1, high_power:1, high_speed:1, intr_multi_block:1;
};
...
struct mmc_host {
	...
	int (*blk_read)(struct mmc_host *host, uint8_t *dst, uint32_t src,
			uint32_t nblks);
	int (*blk_write)(struct mmc_host *host, uint32_t dst, uint8_t *src,
			 uint32_t nblks);
};
...
int mmc_switch(struct mmc_host *host, struct mmc_card *card, u8 set, u8 index,
	       u8 value);
int mmc_read(uint32_t src, uint8_t *dst, int size);
int mmc_write(uint8_t *src, uint32_t dst, int size);
