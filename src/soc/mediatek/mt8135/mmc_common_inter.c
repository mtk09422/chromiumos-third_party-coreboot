...
unsigned char g_storage_buffer[STORAGE_BUFFER_SIZE];
...
enum {
	EMMC_PART_UNKNOWN =
	    0, EMMC_PART_BOOT1, EMMC_PART_BOOT2, EMMC_PART_RPMB, EMMC_PART_GP1,
	    EMMC_PART_GP2, EMMC_PART_GP3, EMMC_PART_GP4, EMMC_PART_USER,
	    EMMC_PART_NUM
};
...
static int mmc_bread(struct blkdev *bdev, u32 blknr, u32 blks, u8 *buf)
{
...
static int mmc_bwrite(struct blkdev *bdev, u32 blknr, u32 blks, u8 *buf)
{
	struct mmc_host *host = (struct mmc_host *)bdev->priv;
